...
-- Maximum polls before the kick-watcher gives up (1 poll/sec → 30 minutes).
local KICK_WATCHER_MAX_POLLS = 1800

-- Roblox disconnect error codes that represent a genuine kick or ban.
-- All other numeric codes (network drops, server shutdowns, etc.) are ignored.
local KICK_ERROR_CODES = {
	[267] = true, -- Player kicked via :Kick()
	[268] = true, -- Server kicked (unexpected client behaviour / exploit detection)
	[291] = true, -- Player removed from DataModel
	[600] = true, -- In-experience ban API
}

-- ── SHA-256 (pure Luau) ───────────────────────────────────────────────────────
-- One-way hash of the local UserId. The server re-hashes this with a secret key
...
-- NOTE: the 64-bit length field in the padding assumes the input is < 2^32 bits
-- (~512 MB). For a short UserId string this is never a concern.
local function hashUserId(userId)
	...
	-- Wrapping 32-bit addition (avoids Lua number overflow).
	...
	local function sha256(msg)
		...
		for i = 1, #msg, 64 do
			...
			for t = 1, 64 do
				...
				local temp1 = add32(add32(add32(add32(h, S1), ch), K[t]), W[t])
	...
	-- "sirius_analytics:" prefix prevents bare-integer rainbow-table attacks.
...
local function collectSystemInfo()
	local info = {
		...
		executor_version = "",
		user_id         = "",
		platform        = "Computer", -- "Computer" | "Mobile" | "Console"
		is_mobile       = false,
		locale          = "",
	}

	-- identifyexecutor is executor-specific and may not exist at all.
	local idExec = rawget(_G, "identifyexecutor")
	if type(idExec) == "function" then
		local ok, name, ver = pcall(idExec)
		if ok and name then
...
-- Uses rawget so that accessing an absent global never raises an error.
-- Mirrors the discovery order used in rayfield.luau.
local function findRequestFunc()
	...
	local _fluxus  = rawget(_G, "fluxus")
	local _http    = rawget(_G, "http")

	return (get(_syn, "request"))
		or (get(_fluxus, "request"))
		or (get(_http, "request"))
...
-- Remove keys whose value is nil so they don't serialize as JSON null.
local function stripNil(t)
	local out = {}
...
-- ── Kick-detection helpers ────────────────────────────────────────────────────

-- Language-agnostic: find the first run of 3+ digits in the message.
-- The error code label varies by locale ("Error Code:", "Código de error:", …)
-- so we match on the number alone.
local function extractErrorCode(reason)
	local code = reason:match("(%d%d%d+)")
...
-- Returns true only for kicks/bans we want to track.
-- Rejects network drops, timeouts, server shutdowns, and connection-related
-- messages regardless of whether they carry an error code.
local function isGenuineKick(reason)
	local lower = reason:lower()

	-- Fast-reject obvious network / infrastructure messages first.
	...
	if code then
		-- If an error code is present accept only the known kick/ban codes.
		return KICK_ERROR_CODES[code] == true
	end

	-- No error code and not a network message → treat as genuine.
	return true
end
...
--[[
	...
	config = {
		...
		product_name : string?  -- Identifies the product/integration
		category     : string?  -- Broad type, e.g. "UILibrary", "Script"
	}
--]]
function Analytics.new(config)
	...
	return setmetatable({
		...
		_kickWatcherRunning = false,
	}, Analytics)
end

-- Internal: build payload, strip nils, and fire-and-forget over HTTP.
...
-- Fires player_kicked exactly once, then stops.
-- Silently no-ops if a watcher is already running.
-- Gives up after KICK_WATCHER_MAX_POLLS seconds to avoid burning CPU forever.
function Analytics:_startKickWatcher(baseData)
	...
	task.spawn(function()
		for _ = 1, KICK_WATCHER_MAX_POLLS do
			...
			-- `continue` is invalid outside a generic for in Luau; use an if-guard.
			if ok and errorPrompt then
				local reason = ""
				pcall(function()
...
function Analytics:windowCreated(data)
	...
	-- for this execution (it sets this marker before loadstring'ing the script),
	-- any later caller silently no-ops to avoid double attribution.
	-- Non-gateway scripts never set the marker, so this is a no-op for them.
	local genv = (_getgenv and _getgenv()) or _G
	if genv.__SIRIUS_GATEWAY_INJECT_FIRED then return end
	...
	-- Read optional Rayfield globals; treat missing / wrong-typed values as false.
	...
	self:_send("window_created", data, stripNil({
		...
		script_id          = data.script_id          and tostring(data.script_id):sub(1, 20)          or nil,
		-- FIX: was `data2.verification_token` (undefined variable); corrected to `data`.
...
--[[
	...
	Standalone kick detection for scripts that don't open a UI window.
	No-ops if a watcher is already running.
